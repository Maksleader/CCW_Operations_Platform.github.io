...
namespace CareProviderPortal.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : CareProviderPortalServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("CareProviderPortal", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
