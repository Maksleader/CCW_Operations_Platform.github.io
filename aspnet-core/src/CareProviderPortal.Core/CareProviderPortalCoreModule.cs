...
using CareProviderPortal.Authorization.Delegation;
using CareProviderPortal.Authorization.Ldap;
using CareProviderPortal.Authorization.Roles;
using CareProviderPortal.Authorization.Users;
using CareProviderPortal.Chat;
using CareProviderPortal.Configuration;
using CareProviderPortal.DashboardCustomization.Definitions;
using CareProviderPortal.Debugging;
using CareProviderPortal.DynamicEntityProperties;
using CareProviderPortal.Features;
using CareProviderPortal.Friendships;
using CareProviderPortal.Friendships.Cache;
using CareProviderPortal.Localization;
using CareProviderPortal.MultiTenancy;
using CareProviderPortal.Net.Emailing;
using CareProviderPortal.Notifications;
using CareProviderPortal.WebHooks;
using Newtonsoft.Json;

namespace CareProviderPortal
{
    [DependsOn(
        typeof(CareProviderPortalCoreSharedModule),
        typeof(AbpZeroCoreModule),
        typeof(AbpZeroLdapModule),
        typeof(AbpAutoMapperModule),
        typeof(AbpAspNetZeroCoreModule),
        typeof(AbpMailKitModule))]
    public class CareProviderPortalCoreModule : AbpModule
    {
        public override void PreInitialize()
        {
            ...
            CareProviderPortalLocalizationConfigurer.Configure(Configuration.Localization);
            ...
            Configuration.MultiTenancy.IsEnabled = CareProviderPortalConsts.MultiTenancyEnabled;
            ...
            Configuration.ReplaceService<IMailKitSmtpBuilder, CareProviderPortalMailKitSmtpBuilder>(DependencyLifeStyle.Transient);
            ...
            Configuration.ReplaceService(typeof(IEmailSenderConfiguration), () =>
            {
                Configuration.IocManager.IocContainer.Register(
                    Component.For<IEmailSenderConfiguration, ISmtpEmailSenderConfiguration>()
                             .ImplementedBy<CareProviderPortalSmtpEmailSenderConfiguration>()
                             .LifestyleTransient()
                );
            });
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(CareProviderPortalCoreModule).GetAssembly());
        }
