...
using CareProviderPortal.Web.IdentityServer;
using CareProviderPortal.Web.Swagger;
using Stripe;
...
using CareProviderPortal.Schemas;
using CareProviderPortal.Web.HealthCheck;
using Newtonsoft.Json.Serialization;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;

namespace CareProviderPortal.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<CareProviderPortalWebHostModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "CareProviderPortal API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("CareProviderPortal.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "CareProviderPortal API", Version = "v1"});
                ...
                if (canShowSummaries)
                {
                    ...
                    var applicationXml = $"CareProviderPortal.Application.xml";
                    var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                    options.IncludeXmlComments(applicationXmlPath);

                    var webCoreXmlFile = $"CareProviderPortal.Web.Core.xml";
