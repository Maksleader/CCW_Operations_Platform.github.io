...
using CareProviderPortal.Test.Base.TestData;
...
namespace CareProviderPortal.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="CareProviderPortalDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(CareProviderPortalDbContext context)
            {
        ...
        protected void UsingDbContext(Action<CareProviderPortalDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CareProviderPortalDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<CareProviderPortalDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<CareProviderPortalDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CareProviderPortalDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CareProviderPortalDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CareProviderPortalDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CareProviderPortalDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<CareProviderPortalDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<CareProviderPortalDbContext, Task<TResult>> func)
        {
            TResult result;
